...
for (var i = 0; i < facesTemp.length; i++) {
  ...
  normalResult = cross(
    subtract(verticeB, verticeA),
    subtract(verticeC, verticeA)
  );
...
var lightAmbient = vec4(0.05, 0.023, 0, 1.0);
var lightDiffuse = vec4(1, 0.717, 0, 1);
var lightSpecular = vec4(1, 0.823, 0.29, 1.0);
...
function getLighting() {
  ...
  var ambientUniformPositionLocation = gl.getUniformLocation(
    program,
    "ambientColour"
  );
  gl.uniform4fv(ambientUniformPositionLocation, lightAmbient);
  var diffuseUniformPositionLocation = gl.getUniformLocation(
    program,
    "diffuseColour"
  );
  gl.uniform4fv(diffuseUniformPositionLocation, lightDiffuse);
  var specularUniformPositionLocation = gl.getUniformLocation(
    program,
    "specularColour"
  );
